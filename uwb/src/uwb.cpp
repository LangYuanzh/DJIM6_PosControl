...
int main(int argc, char *argv[])  
{
     ...
     uwbposPub = nh.advertise<geometry_msgs::PointStamped>("uwb/uwb_position", 10);     
     uwbvelPub = nh.advertise<geometry_msgs::TwistStamped>("uwb/uwb_velocity", 10);     
	...
	while(1)
    {
        ...
        uwbpos.point.x = tag0.tag_position.x/1000.0;
        uwbpos.point.y = tag0.tag_position.y/1000.0;
        uwbpos.point.z = tag0.tag_position.z/1000.0;
        
        uwbvel.twist.linear.x = tag0.tag_velocity.x/10000.00;
        uwbvel.twist.linear.y = tag0.tag_velocity.y/10000.00;
        uwbvel.twist.linear.z = tag0.tag_velocity.z/10000.00;

        uwbposPub.publish(uwbpos);
        uwbvelPub.publish(uwbvel);
        gettimeofday(&tn,NULL);
        //ROS_INFO("%d",tn.tv_usec - t.tv_usec);
